...
public class AutoTableBindPlugin extends ActiveRecordPlugin {
    ...
    public boolean start() {
        ...
        if(super.start()){
        	for (Class modelClass : modelClasses) {
        		tb = (TableBind) modelClass.getAnnotation(TableBind.class);
        		if(tb != null && tb.fuzzyQuery() != null && tb.fuzzyQuery().length > 0){
                	Map<String, Class<?>> columns = TableMapping.me().getTable(modelClass).getColumnTypeMap();
            		for(String str : tb.fuzzyQuery()){
            			if(!columns.containsKey(str)){
            				throw new RuntimeException("Can not setting funzzyQuery, then column["+str+"] is not exists in the table["+tb.tableName()+"].");
            			}
            		}
                }
        	}
